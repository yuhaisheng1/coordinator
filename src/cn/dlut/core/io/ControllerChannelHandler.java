...
import cn.dlut.exceptions.ControllerStateException;
...
import cn.dlut.util.OVXMessageUtil;
...
public class ControllerChannelHandler extends OFChannelHandler {
    ...
    enum ChannelState {
        ...
        ACTIVE(true) {
            ...
            void processOFMessage(final ControllerChannelHandler h,
                ...
                case GET_CONFIG_REQUEST:
                	//h.log.info("received packet: {}", m.getType());
                    h.sw.handleIO(m, h.channel);
                    break;
                case VENDOR:
        ...
        void processOFGetConfigRequest(final ControllerChannelHandler h,
                final OFGetConfigRequest m) throws IOException {
            this.illegalMessageReceived(h, m);
        }
    ...
    public void messageReceived(final ChannelHandlerContext ctx,
        ...
        if (e.getMessage() instanceof List) {
            ...
            for (final OFMessage ofm : msglist) {
                ...
                try {
                	/*System.out.println("switch receive "+ofm.getType()+"from "+ctx.getChannel().getRemoteAddress());
                	switch(ofm.getType()) {
                	case STATS_REQUEST:
                		System.out.println("switch receive "+((OFStatisticsRequest)ofm).getStatisticType()+"from"+ctx.getChannel().getRemoteAddress());
					default:
						break;
                	}*/
                    switch (ofm.getType()) {
                    case PACKET_OUT:
                        ...
                        if (data.length >= 14) {
                            if (OVXLLDP.isLLDP(data)) {
                            	//OVXSwitch ¥¶¿ÌLLDP      
                                ((OVXSwitch)this.sw).handleLLDP(ofm, this.sw);
                                break;
                            }
                        }
                    default:
