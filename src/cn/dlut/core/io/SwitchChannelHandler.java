...
import cn.dlut.exceptions.HandshakeTimeoutException;
import cn.dlut.exceptions.SwitchStateException;
import cn.dlut.packet.OVXLLDP;
...
public class SwitchChannelHandler extends OFChannelHandler {
    ...
    enum ChannelState {
        ...
        WAIT_DESCRIPTION_STAT_REPLY(false) {
            ...
            void processOFStatisticsReply(final SwitchChannelHandler h,
                ...
                final OFDescriptionStatistics description = new OFDescriptionStatistics();
                ...
                if(!(OpenVirteXController.ctrls.isEmpty()))
                {
                	System.out.println("controller is  not empty +++++++++++++++========");
                	Set<PhysicalSwitch> switches=new HashSet();
                    switches.add((PhysicalSwitch)h.sw);
                    OpenVirteXController.getInstance().addControllers(OpenVirteXController.ctrls,switches);
                }                
                ...
                h.sw.boot();
                h.setState(ACTIVE);
                //h.log.info("DPID: {}, switch description ok+++++++++++++++++++++++", h.sw.getSwitchId());
                //h.log.info("DpID: {}, ChannelLocalAddress: {}, ChannelRemoteAddress: {}", h.featuresReply.getDatapathId(), h.channel.getLocalAddress(),h.channel.getRemoteAddress());
            }
    ...
    private void sendHandshakeSetConfig() throws IOException {
        ...
        configSet.setMissSendLength(OFSetConfig.MSL_FULL).setLengthU(
    ...
    public void messageReceived(final ChannelHandlerContext ctx,
        ...
        if (e.getMessage() instanceof List) {
            ...
            for (final OFMessage ofm : msglist) {
                ...
                try {
                	/*System.out.println("switch receive "+ofm.getType());
                	switch(ofm.getType()) {
                	case STATS_REPLY:
                		System.out.println("switch receive "+((OFStatisticsReply)ofm).getStatisticType());
					default:
						break;
                	}*/
                    switch (ofm.getType()) {
